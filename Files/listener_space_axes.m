classdef listener_space_axes < handle
    ...
    methods
        function obj = listener_space_axes(fig) %the space, including the grid
            obj.main_axes = fig;
            grid(fig,'on')
            xlabel(fig, 'x -> [m]')
            ylabel(fig, 'y -> [m]')
            xlim(fig,[-3,3]);   %expand this
            ylim(fig,[-3,3]);   %and this to make the simulation space bigger
      %      axis equal tight
        end
        
        function receiver = draw_head(obj,pos,R) 
            N = 20;                         %number of obejcts the head is made of
            fi = (0:2*pi/N:2*pi*(1-1/N));   %patch object = made out of polygons
            x_head = cos(fi)*R;
            y_head = sin(fi)*R;
            fi_nose =  [10 5   0    -5  -10 ];
